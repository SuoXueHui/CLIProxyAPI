...
- [ ] Write failing tests for defaults, accepted values, rejected values, parsing parity, and config change details.
- [ ] Run `go test ./internal/config ./internal/watcher/diff` and confirm the new tests fail for missing symbols/behavior.
- [ ] Implement the config type, defaults, validation, public SDK aliases, example YAML, and redacted diff entries.
- [ ] Run `gofmt -w` on changed Go files and rerun the focused tests until green.
- [ ] Commit the independently passing configuration contract.
...
- [ ] Write table-driven failing tests for disabled, OAuth gating, malformed/oversized body, the three eligible tails, unsupported tails, stable canary, `1/2/4` pairs, and historical/core marker idempotency.
- [ ] Add assertions that skipped results reuse the original backing slice and injected bodies contain matched call/output IDs.
- [ ] Run `go test ./internal/runtime/executor/helps -run CodexWeeklyOverdraft` and confirm the expected failures.
- [ ] Implement the stateless transform with stable SHA-256 bucketing, deterministic call IDs, one final JSON replacement, and atomic counters.
- [ ] Add outcome classification tests for success, 429, 401/402/403, cancellation, and other errors.
- [ ] Run focused tests and `go test -race ./internal/runtime/executor/helps -run CodexWeeklyOverdraft` until green.
- [ ] Run the disabled/skip benchmarks and confirm zero replacement-body allocations.
- [ ] Commit the independently passing transform and metrics.
...
- [ ] Write failing HTTP tests that capture the upstream request and assert observe mode is unchanged while inject mode appends exactly one pair.
- [ ] Write failing WebSocket tests that capture `response.create` and assert one pair remains one pair after a resend.
- [ ] Run `go test ./internal/runtime/executor -run WeeklyOverdraft` and confirm the expected payload assertions fail.
- [ ] Integrate the helper after reasoning replay and before cache/identity processing in all four paths.
- [ ] Record non-stream outcomes on function return and stream outcomes at terminal event/error without changing retry ownership.
- [ ] Run focused executor tests and `go test -race ./internal/runtime/executor -run WeeklyOverdraft` until green.
- [ ] Commit the independently passing transport integration.
...
- [ ] Write a failing handler test for effective config, counters, and absence of credential/request fields.
- [ ] Run `go test ./internal/api/handlers/management -run CodexWeeklyOverdraft` and confirm failure.
- [ ] Implement the read-only handler and register the route.
- [ ] Run focused management and API route tests until green.
- [ ] Commit the independently passing status endpoint.
...
- [ ] Run all focused package tests.
- [ ] Run `go test -race ./internal/runtime/executor/helps ./internal/runtime/executor`.
- [ ] Run `go test ./...`.
- [ ] Run `go build -o test-output ./cmd/server && rm test-output`.
- [ ] Inspect `git diff --check`, `git status`, and the complete branch diff for secrets, body retention, duplicate injection, or scheduler/retry changes.
- [ ] Update planning and durable project knowledge only with verified facts.
- [ ] Commit final documentation/knowledge updates and report the branch, commits, verification, rollout order, and remaining experimental risk.
