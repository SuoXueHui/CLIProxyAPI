...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.UsageOutboxPath != newCfg.UsageOutboxPath {
		changes = append(changes, fmt.Sprintf("usage-outbox-path: %s -> %s", strings.TrimSpace(oldCfg.UsageOutboxPath), strings.TrimSpace(newCfg.UsageOutboxPath)))
	}
